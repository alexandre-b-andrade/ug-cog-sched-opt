...
try:
    ...
    print('NPV calculation was not possible')
    result_makespan = 'NA'
try:    
    ...
    print('GAP calculation was not possible')
    result_gap = 'NA'
...
try:
    ...
    print('Output stats calculation was not possible')
...
out_dir = curr_dir + "/" + "Results"
lp_file = str(mip_stats_output[0][4]) + str('-') + str(mip_stats_output[0][5]) + str('-') + str(len(input_tasks)) + str('_tasks-') + str(len(input_machines)) + str('_mach-') + time.strftime("%Y_%m_%d-%H%M%S") # Solver + Algorithm + # tasks + # machines + Datetime
mdl.export_as_lp(path = out_dir, basename = lp_file)
...
try:
    ...
    print('Best bound calculation was not possible')
